...
#define OGRE_DELETE_T(ptr, T, category) shh_delete ptr;
    ...
    String DataStream::getAsString(void)
    {
		...
		if (!eof())
		{
			size_t nr = read(pBuf, 3);
			if (3 == nr)
			{
				if (U8HEAD0 == (unsigned char)pBuf[0] && U8HEAD1 == (unsigned char)pBuf[1] && U8HEAD2 == (unsigned char)pBuf[2])
				{
					//utf8
				}
				else
        ...
        : DataStream(static_cast<uint16>(readOnly ? READ : (READ | WRITE)))
    {
        ...
        if (mSize == 0 && !sourceStream.eof())
        {
            ...
            mSize = contents.size();
            mData = OGRE_ALLOC_T(byte, mSize, MEMCATEGORY_GENERAL);
            mPos = mData;
	...
	//-----------------------------------------------------------------------
	void MemoryDataStream::flush(void)    
	{
		//empty
	}
        ...
        const String& delim)
    {
		if (delim.empty())
		{
			shh_error("No delimiter provided :FileStreamDataStream::readLine");
			//OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "No delimiter provided",
			//	"FileStreamDataStream::readLine");
		}
		if (delim.size() > 1)
		{
			shh_error("WARNING: FileStreamDataStream::readLine - using only first delimeter");
		...
		else if (mpInStream->fail())
		{
			...
			{
				shh_error("Streaming error occurred: FileStreamDataStream::readLine");
	...
	//-----------------------------------------------------------------------
	void FileStreamDataStream::flush(void)    
	{
		if (mpInStream)
		{
			if (mpFStream)
	...
	//-----------------------------------------------------------------------
	void FileHandleDataStream::flush(void)    
	{
		if (mFileHandle != 0)
		{
			fflush(mFileHandle);
